...
% save('elecCoords_v14_32x32.mat', 'elecCoords_v14_32x32');
...
pkIndex = [];
fs = 20000;
t1 = 100%190; % stim start (s)
t2 = 115%201; % stim end (s)
th = 15;
...
j = 1;
 for i = 1:ch_num
     ...
     if(impedanceMag(i)<500e3 && impedanceMag(i)>1e3) 
...
timeStart = -0.1;
timeInterval = 0.5;
numTrials = length(pkIndex)-1;

% Real trial averaging loop
ampDataTA = zeros(ch_num,fs*timeInterval+1);
    y_bpf = zeros(numTrials, ch_num, int32(timeInterval*fs)+1);
...
ampDataTA = ampDataTA + reshape(y_bpf(i,:,:), [ch_num, 10001]);
...
% %     end
    ...
    for x_pos = -23.25:1.5:23.25%-15.5:1:15.5;
...
% 	data = ampDataTA;
% 	data = ddf;
	offset = 3*std(reshape(data, [ch_num*length(data),1]));
    num_ln = length(k_temp);
      for i = 1:num_ln
        t = 1e3*((1:length(data(1,:)))/fs+timeStart);
%                 yline(offset*i, 'b');
%                 hold on;
        plot(t,offset*i+data(k_sorted(i,2),:), 'k')   
        hold on;
      end
...
% 	xlim([0 50]);
    ...
    txt_filename = append('CS_SSEP_Lpost_May14_2021_', num2str(t1), 's_', num2str(x_pos), 'mm_', '.png');;
...
%     xline(15);
   ...
   %print('-dpng','-r300',txt_filename);
    %%  (ignore this 2 - differentiation of the waveforms)
    df = [];
    ddf = [];
    for i = 1:ch_num
        ...
        %% Data (ignore this 3 - All waveforms)
    data = [];
    for i = 1:ch_num
...
%         data = ddf;
    ...
    for time_ms = 15:30%[10:1:50, 60:10:100]
    ref_ms = 25;
    time = time_ms * 1e-3;
...
%     axis([-ax_scl+1 ax_scl -ax_scl+1 ax_scl]);
    ...
    txt_filename = append('CS_May14_2021_', num2str(t1), 's_',num2str((time)*1000, '%.0fms'), '.png');;
    print('-dpng','-r300',txt_filename);
...
%     plot(data(1,:))
    ...
    pixel_size = 80;
    ave_data = [];
    range = 0.205*fs:0.300*fs;
    for i = 1:ch_num
%        ave_data(i,:) = mean(data(i,specific_index-300:specific_index+300)); 
       tmp = corrcoef(data(k(selected_k_index),range), data(i,range));
       ave_data(i) = tmp(1,2);
%         ave_data(i,:) = data(i, specific_index) - REF;
    end
...
%     scatter(elecX(k),elecY(k),pixel_size,data(k,specific_index)-REF,'filled', 'o'); 
    scatter(elecX(k),elecY(k),pixel_size,ave_data(k),'filled', 'o');
    hold on;
    k_tmp = k(selected_k_index);
    scatter(elecX(k_tmp),elecY(k_tmp),pixel_size*2,ave_data(k_tmp),'filled', 'o');
    box on;
    ax=gca;ax.LineWidth=1.5;
    set(gca,'fontsize', 18);
        set(gca,'fontweight', 'bold');
...
%     title(txt);
    ...
    txt_filename = append('CCCoeff_May14_2021_', num2str(t1), 's_',num2str((time)*1000, '%.0fms'), '.png');;
    print('-dpng','-r300',txt_filename);
...
%     cm = colormap(jet); 
    t_peak_ds = 100+23; %ms
    l_width = 0.5;
    max_amp = max(max(abs(raw_TA_data(k,t_peak_ds))));
...
%     axis off;
    ...
    txt = 'CS_mapping_May14_2022_P20N20';
    print('-dpng','-r300',txt);
